  ...
  return (
    <div className="space-y-8">
      ...
      <section className="grid gap-4 md:grid-cols-4">
        <SummaryTile label="Quotes" value={String(quotes.length)} />
        <SummaryTile label="Quoted value" value={formatCurrency(summary.totalQuoted)} />
        <SummaryTile label="Paid" value={formatCurrency(paidTotal)} />
        <SummaryTile
          label="Points"
          value={
              ...
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-soil/60">
                    Use this as the support reference when the same client asks
                    for the same address, service, and scope.
                  </p>
                </div>
                <StatusBadge status={referenceQuote.status} />
              </div>

              <dl className="grid gap-4 border-t border-oak/10 pt-5 text-sm md:grid-cols-4">
                <DetailItem label="Date" value={formatDate(referenceQuote.created_at)} />
                <DetailItem
                  label="Service"
                  value={referenceQuote.service_name ?? "Not recorded"}
                />
                <DetailItem
                  label="Address"
                  value={referenceQuote.address ?? "Not recorded"}
                />
                <DetailItem
                  label="Frequency"
      ...
      <section className="grid gap-5 lg:grid-cols-[1fr_1.4fr]">
        <Card>
          ...
          <dl className="mt-5 space-y-4 text-sm">
            <DetailItem label="Profile ID" value={profile?.id ?? "Quote-only record"} />
            <DetailItem label="Role" value={summary.role} />
            <DetailItem label="Name" value={profile?.full_name ?? summary.name} />
            <DetailItem label="Email" value={summary.email ?? "Not provided"} />
            <DetailItem label="Phone" value={summary.phone ?? "Not provided"} />
            <DetailItem
              label="Created"
        ...
        <Card>
          ...
          <dl className="mt-5 grid gap-4 text-sm sm:grid-cols-2">
            <DetailItem label="Last request" value={formatDate(summary.latestQuoteAt)} />
            <DetailItem
              label="Last status"
              value={summary.latestStatus ?? "No requests"}
            />
            <DetailItem
              label="Last service"
              value={summary.latestService ?? "Not recorded"}
            />
            <DetailItem
              label="Last address"
              value={summary.latestAddress ?? "Not recorded"}
            />
            <DetailItem label="Payments" value={String(payments.length)} />
            <DetailItem label="Contracts" value={String(contracts.length)} />
          </dl>
        </Card>
      </section>

      <section>
        <CardTitle>Past quotes</CardTitle>
        <div className="mt-4 space-y-4">
          {!quotes.length ? (
            <EmptyState
              title="No quote history"
              body="This account does not have any quote requests yet."
            />
          ) : (
      ...
      <section className="grid gap-5 lg:grid-cols-2">
        ...
        <RelatedList
          title="Rewards"
          empty="No rewards ledger entries."
          items={rewards}
  ...
  return (
    <Card>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="font-display text-2xl text-oak">
            {quote.address ?? "Quote request"}
          </p>
          <p className="mt-1 text-sm text-soil/50">
            {formatDate(quote.created_at)} - {quote.service_name ?? "Service not recorded"}
          </p>
        </div>
      ...
      <dl className="mt-5 grid gap-4 border-t border-oak/10 pt-5 text-sm sm:grid-cols-2 lg:grid-cols-4">
        <DetailItem label="Requester" value={quote.requester_name ?? "Not provided"} />
        <DetailItem label="Contact email" value={quote.requester_email ?? "Not provided"} />
        <DetailItem label="Account email" value={quote.account_email ?? "Not available"} />
        <DetailItem label="Phone" value={quote.requester_phone ?? "Not provided"} />
        <DetailItem label="Frequency" value={quote.frequency ?? "Not selected"} />
        <DetailItem label="Source zone" value={quote.source_zone ?? "Not recorded"} />
        <DetailItem
          label="Notification"
          value={quote.notification_status ?? "Not recorded"}
        />
...
function ContractRowView({ contract }: { contract: ContractRow }) {
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 border-t border-oak/10 pt-3 first:border-t-0 first:pt-0">
      <div>
        <p className="text-sm font-medium text-oak">
          {contract.quote_id ? `Quote ${contract.quote_id.slice(0, 8)}` : "Contract"}
        </p>
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 border-t border-oak/10 pt-3 first:border-t-0 first:pt-0">
      ...
      <span className="rounded-full bg-sand/40 px-3 py-1 text-xs text-soil/70">
        {referral.reward_granted ? "Reward granted" : "Pending reward"}
      </span>
    </div>
  );
}
